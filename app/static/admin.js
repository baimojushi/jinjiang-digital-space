...
function renderContent(items){$("#contentBody").innerHTML=items.map(x=>`<tr><td><div class="work"><img src="${x.cover||""}"><div><b>${esc(x.title)}</b><small>${esc(x.asset_code||"")}</small></div></div></td><td>${esc(x.theme)}</td><td>${x.exposures}</td><td>${x.details}</td><td>${x.likes}</td><td>${x.favorites}</td><td>${x.votes}</td><td><b>${x.interest_score}</b></td></tr>`).join("")}
function renderCandidates(pool){
 $("#signalList").innerHTML=(pool.user_signal_items||[]).slice(0,12).map(x=>`<div class="candidate"><img src="${x.cover||""}"><div><b>${esc(x.title)}</b><span>${esc(x.theme)} · ${x.exposures}次曝光 · ${x.curation_votes}票</span></div><div class="score">${x.interest_score}<small>兴趣热度</small></div></div>`).join("")||`<div class="empty">还没有用户行为数据。</div>`;
 $("#internalList").innerHTML=(pool.internal_candidates||[]).slice(0,18).map(x=>`<div class="candidate"><img src="${x.cover||""}"><div><b>${esc(x.title)}</b><span>${esc(x.collection_name||"内部资源")} · ${esc(x.building||"楼宇待定")}</span></div><div class="score" style="font-size:10px">${esc(x.rights_status)}<small>${esc(x.readiness)}</small></div></div>`).join("");
}
...
function renderProposal(p){
 ...
 $("#proposalBox").innerHTML=`<span class="eyebrow">${esc(p.theme)}</span><h3>${esc(p.title)}</h3><p>${esc(p.statement)}</p><div class="proposal-works">${p.works.map(w=>`<figure><img src="${w.cover||""}"><figcaption>${esc(w.title)} · ${w.votes}票</figcaption></figure>`).join("")}</div><p>${esc(p.status)}。发布动作会写入 Exhibition / ExhibitionAsset / Activity，并回流到用户端“正在发生”。</p>`;
