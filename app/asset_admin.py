...
def data_quality():
    ...
    for r in con.execute("SELECT * FROM culture_assets ORDER BY id"):
        ...
        if d["rights_status"]=="pending":
            issues.append({"severity":"blocking","entity":"asset","id":d["id"],"code":d["asset_code"],"field":"rights_status","message":"授权状态待确认，禁止公开发布"})
        if not d["source"]:
...
def recompute_space_matches():
    ...
    for a in assets:
        ...
        for s in spaces:
            ...
            rights=1.0 if a["rights_status"] in ("authorized","public_domain_verified") else (0.45 if a["rights_status"]=="pending" else 0.25)
            uh=heat.get(a["id"],0)/max_heat
            score=round(b*.35+th*.25+sty*.15+rights*.15+uh*.10,4)
            readiness="blocked_by_space_metadata" if missing_space else ("ready" if rights>=1 else "rights_blocked")
